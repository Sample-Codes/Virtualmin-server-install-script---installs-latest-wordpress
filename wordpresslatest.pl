# script_wordpresslatest_desc()
sub script_wordpresslatest_desc
{
return "WordPress Latest";
}

sub script_wordpresslatest_uses
{
return ( "php" );
}

sub script_wordpresslatest_longdesc
{
return "Latest Wordpress";
}

# script_wordpresslatest_versions()
sub script_wordpresslatest_versions
{
return ( "Latest" );
}

sub script_wordpresslatest_category
{
return "Blog";
}

sub script_wordpresslatest_php_vers
{
local ($d, $ver) = @_;
	return ( 5 );
}

sub script_wordpresslatest_php_modules
{
return ("mysql", "gd");
}

sub script_wordpresslatest_php_optional_modules
{
return ("curl");
}

sub script_wordpresslatest_dbs
{
return ("mysql");
}

# script_wordpresslatest_depends(&domain, version)
sub script_wordpresslatest_depends
{
local ($d, $ver) = @_;
local @rv;
...
# script_wordpresslatest_params(&domain, version, &upgrade-info)
# Returns HTML for table rows for options for installing Wordpress
sub script_wordpresslatest_params
{
...
# script_wordpresslatest_parse(&domain, version, &in, &upgrade-info)
# Returns either a hash ref of parsed options, or an error string
sub script_wordpresslatest_parse
{
...
# script_wordpresslatest_check(&domain, version, &opts, &upgrade-info)
# Returns an error message if a required option is missing or invalid
sub script_wordpresslatest_check
{
...
# script_wordpresslatest_files(&domain, version, &opts, &upgrade-info)
# Returns a list of files needed by Wordpress, each of which is a hash ref
# containing a name, filename and URL
sub script_wordpresslatest_files
{
local ($d, $ver, $opts, $upgrade) = @_;
local @files = ( { 'name' => "source",
	   'file' => "wordpress-latest.zip",
...
sub script_wordpresslatest_commands
{
return ("unzip");
}

# script_wordpresslatest_install(&domain, version, &opts, &files, &upgrade-info)
# Actually installs WordPress, and returns either 1 and an informational
# message, or 0 and an error
sub script_wordpresslatest_install
{
...
# script_wordpresslatest_uninstall(&domain, version, &opts)
# Un-installs a Wordpress installation, by deleting the directory and database.
# Returns 1 on success and a message, or 0 on failure and an error
sub script_wordpresslatest_uninstall
{
...
# script_wordpresslatest_realversion(&domain, &opts)
# Returns the real version number of some script install, or undef if unknown
sub script_wordpresslatest_realversion
{
...
# script_wordpresslatest_latest(version)
# Returns a URL and regular expression or callback func to get the version
sub script_wordpresslatest_latest
{
local ($ver) = @_;
...
sub script_wordpresslatest_site
{
